...
namespace ClearBG.Editor
{
    public class ClearBgSettingsWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            if (!Mathf.Approximately(position.width, _windowSize.x) || !Mathf.Approximately(position.height, _windowSize.y))
            ...
            {
                if (EditorUtility.DisplayDialog("Convert All Canvases", "This will add ClearBGCanvas component to all Canvas objects in the current scenes and all prefabs in the project. Are you sure you want to proceed?", "Yes", "No"))
                {
                    ConvertAllCanvases();
                }
            }
